...
- конструктор по кнопке «Добавить агента»: восемь готовых кабинетов, восемь
  аватаров и обязательный system prompt; профиль появляется без изменения кода;
...
`localStorage` текущего браузера под ключом `agent-bureau.custom-agents.v1`.
System prompt не попадает в collector, task summary или HTML карточки. Такой
профиль уже готов как конфигурация интерфейса, но статический Vercel-сайт не
запускает модель сам: для автономной работы потребуется локальный runtime adapter.
Удалить созданный профиль можно из его карточки.
