...
- Конструктор профиля: кнопка «Добавить агента» предлагает 8 переиспользуемых
  кабинетов и 8 аватаров, принимает system prompt, сохраняет до 40 профилей в
  `localStorage` и сразу добавляет их в цифровой аннекс/пул
...
2. Добавить локальный runtime adapter, который безопасно читает выбранный
   пользовательский профиль и запускает соответствующую модель/агента.
